...
function Chores() {
  ...
  const fetchAll = async () => {
    try {
      ...
      const [choresRes, lifecyclesRes, ratesRes] = await Promise.all([
        api.get('/admin/chores'),
        api.get('/admin/lifecycles'),
        api.get('/admin/rates')
      ]);
  ...
  const handleChoreSubmit = async (e) => {
    ...
    try {
      if (choreFormData.id) {
        await api.put(`/admin/chores/${choreFormData.id}`, choreFormData);
      } else {
        await api.post('/admin/chores', choreFormData);
      }
  ...
  const handleDeleteChore = async (id) => {
    ...
    try {
      await api.delete(`/admin/chores/${id}`);
      fetchAll();
    } catch (err) {
  ...
  const handleToggleEnabledChore = async (chore) => {
    try {
      ...
      await api.put(`/admin/chores/${chore.id}`, { ...chore, enabled: !chore.enabled }); 
      fetchAll();
    } catch (err) {
  ...
  const handleLifecycleSubmit = async (e) => {
    ...
    try {
      ...
      if (lifecycleFormData.id) {
        await api.put(`/admin/lifecycles/${lifecycleFormData.id}`, data);
      } else {
        await api.post('/admin/lifecycles', data);
      }
  ...
  const handleDeleteLifecycle = async (id) => {
    ...
    try {
      await api.delete(`/admin/lifecycles/${id}`);
      fetchAll();
    } catch (err) {
  ...
  const handleRateSubmit = async (e) => {
    ...
    try {
      ...
      if (rateFormData.id) {
        await api.put(`/admin/rates/${rateFormData.id}`, data);
      } else {
        await api.post('/admin/rates', data);
      }
