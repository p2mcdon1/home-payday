...
function Service() {
  ...
  const fetchPendingEfforts = async () => {
    try {
      setLoading(true);
      const response = await api.get('/adult/pending-efforts');
  ...
  const fetchAccountsForUser = async (userId) => {
    try {
      const response = await api.get(`/adult/kids/${userId}/accounts`);
  ...
  const handleWithdrawalSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/adult/withdrawals', {
  ...
  const handleApprove = async (effortId) => {
    try {
      await api.post(`/adult/efforts/${effortId}/approve`);
  ...
  const handleDeny = async (effortId) => {
    ...
    try {
      await api.post(`/adult/efforts/${effortId}/deny`);
