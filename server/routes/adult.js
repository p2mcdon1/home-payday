  ...
  async (req, res) => {
    try {
      ...
      const result = await db.query(
        ...
        [name, password, role, avatarBase64]
      );
...
router.post('/chores',
  ...
  async (req, res) => {
    try {
      ...
      const { name, description, link, enabled, lifecycleId, rateId, avatar } = req.body;
      ...
      const result = await db.query(
        ...
        [name.trim(), description || null, link || null, enabled !== undefined ? enabled : true, lifecycleId, rateId, avatarBase64]
      );
...
router.put('/chores/:id',
  ...
  async (req, res) => {
    try {
      ...
      // Process avatar if provided
      // undefined = don't change, null or '' = clear, string = update
      let avatarBase64 = undefined;
      if (avatar !== undefined) {
        if (avatar === null || avatar === '') {
          avatarBase64 = ''; // Clear avatar (empty string signals clear)
        } else {
          try {
            avatarBase64 = await avatarUtils.processAvatarData(avatar);
          } catch (error) {
      ...
      const result = await db.query(
        ...
        [req.params.id, name.trim(), description || null, link || null, enabled !== undefined ? enabled : true, lifecycleId, rateId, avatarBase64]
      );
